  ...
  def notas_estructuradas resumen, url=nil, informe_id=nil
    ...
    mensaje += enlaces_notas resumen 
		...
		sub_resumenes.each do |sub_resumen|
		  ...
		  mensaje += link_to "separar", {:controller => 'resumenes', :action => "separar", :id => sub_resumen.id, :informe_id => informe_id},  {:class => 'btn btn-mini'} if action_name.eql? "paso2" or action_name.eql? "paso5"	#if (not informe_id.nil? and action_name!="paso4")
